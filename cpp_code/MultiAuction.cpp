...
namespace pl
{
	...
	void pl::MultiAuction::auction()
	{
		...
		size_t circleTime = 0;
		size_t maxcircleTime = _mainMap.getReachableVertNum() * 2;
	...
	bool MultiAuction::calAucVertID(size_t const & aucNeer, size_t & aucVertID)
	{
		...
		for (size_t i = 0; i < robOpNeiSet.size(); i++)
		{
			//if (calOpPriority(aucNeer,)
			//	calPriority(robID, robOpNeiSet.at(size - 1 - i).first) != std::numeric_limits<double>::max())
			//{
